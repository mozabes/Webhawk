...
fn mapWorld(p: vec3<f32>) -> SDFResult {
  ...
  // Aircraft - simple sphere (material 9)
  let aircraft = sdSphere(p - uniforms.aircraftPos, 2.0);
  if (aircraft < result.dist) {
    result.dist = aircraft;
    result.materialId = 9;
  }
